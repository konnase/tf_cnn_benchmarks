...
class BenchmarkCNN(object):
  ...
  def __init__(self, params, dataset=None, model=None):
    ...
    if self.job_name:
      ...
      if use_ps_server:
        ps_tasks = self.cluster_manager.num_ps()
        ps_strategy = tf.contrib.training.GreedyLoadBalancingStrategy(ps_tasks,
  ...
  def benchmark_with_session(self, sess, supervisor, graph_info,
    ...
    while not done_fn():
      ...
      def sigterm_handler(sig_num, frame):
           log_fn('Capture SIGTERM, save model')
           if (self.params.save_model_steps and
                   local_step > 0 and
                   is_chief):
               supervisor.saver.save(sess, supervisor.save_path,
      ...
      # capture sigterm and save the model
